...
void Liner::startLiner(){
	...
	while (1) {
		...
		if (line_1.size() != 0 && line_2.size() != 0) {//1,2번 모두 라인이 존재
			if (modes)
				cout << "1,2모두 검출" << endl;
			...
			if (line_1.size() == 0 && line_2.size() == 0) {//다안검출
				if (modes)
					cout << "1,2다 안 검출" << endl;
				...
				if (modes)
					cout << "1,2중 하나 검출" << endl;
				float angle;
				if (line_1.size() != 0) { //1번만 검출
					angle = AvgLineAngle(line_1);
				}
				else
					if (line_2.size() != 0) {//2번만 검출
						angle = AvgLineAngle(line_2);
					}

				if ((angle > 1 && angle < 89) || (angle > 181 && angle < 269)) {//오른쪽으로
					Avgpt.x = 600;
					Avgpt.y = 300;
				}
				else
					if ((angle > 91 && angle < 179) || (angle > 271 && angle < 359)) {//왼쪽으로
						Avgpt.x = 40;
						Avgpt.y = 300;
					}
